...
sprintf("You generated %d numbers that sum to %d.", count, total)
```
...
result <- calculate_bmi(height = 65, weight = 150)

rounded_bmi <- round(result$bmi, 1)
bmi_category <- result$category

print(sprintf("Your BMI is %s with a value of %.1f.", bmi_category, rounded_bmi))
...
new_list <- list(vec1 = vec1, vec2 = vec2, vec3 = vec3)
print(new_list) 
```
...
is_sum_even <- sapply(new_list, function(x) sum(x) %% 2 == 0)
print(is_sum_even)
```
...
red_fruits <- fruit_info[fruit_info$color == "red", ]
print(red_fruits)
```
...
for (i in 1:nrow(fruit_info)) {
  heavy_or_light <- ifelse(fruit_info$weight[i] > 100, "heavy", "light")
  print(sprintf("The %s is %s.", fruit_info$fruit[i], heavy_or_light))
}
